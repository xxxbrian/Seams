...
def test_notification_in_channel(login_list, channel_list):
    ...
    assert response_1['notifications'][1]['channel_id'] == channel_list[0]['channel_id']
    assert response_1['notifications'][1]['dm-id'] == -1
    assert response_1['notifications'][1]['notification_message'] == "steveyang added you to Steve's channel"
    # Because user[1] has been added to DM[0] at fixture, now the notification should be number 1
    ...
    response_3 = requests.get(url + 'notifications/get/v1',
                              params={'token': login_list[1]['token']}).json()
    assert response_3['notifications'][2]['channel_id'] == channel_list[0]['channel_id']
    assert response_3['notifications'][2]['dm-id'] == -1
    assert response_3['notifications'][2]['notification_message'] == "steveyang reacted to your message in Steve's channel"
...
def test_notification_in_DM(login_list, dm_list):
    ...
    assert response_1['notifications'][0]['dm-id'] == dm_list[0]['dm_id']
...
def test_search_normal(login_list, dm_list, channel_list):
    ...
    '''
    # 3 users send the same messages in their channels for further tests
    for i in range(3):
        requests.post(url + 'message/send/v1',
                      json = {'token': login_list[i]['token'],
                              'channel_id': channel_list[i]['channel_id'],
                              'message': 'Hello guys'})
